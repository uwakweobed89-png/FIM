    ...
    def run_active_mode(self) -> None:
        ...
        while self.running:
            ...
            if os.name == 'nt':  # windows
                new_lines = self.read_windows_log()
            else:
...
# =================================================================
# WINDOWS LOG READING (PLACEHOLDER)
# =================================================================

    def read_windows_log(self):
        """Read Windows Event Log for login events (placeholder)."""
        # This is a placeholder for Windows Event Log reading logic.
        # In a production system, you would implement this using
        # win32evtlog or a similar library to read Security logs.
        logging.info("Windows log reading not implemented yet.")
        server = 'localhost'   # local machine
        log_type = 'Security'  # security logs
        ...
        log_line = []
        for event in events:
            if event.EventID in [4624, 4625]:   # Logon success and failure
                timestamp = str(event.TimeGenerated)

                ip = "unknown"
                user = "unknown"

                log_line = f"{timestamp} Failed login user={user} ip={ip}"
                log_line.append(log_line)
        return log_line
#  ==================================================================
#  MAIN RUN METHOD
#  ==================================================================
